...
class RegisterView(APIView): #API endpointi 
...
class LogoutView(APIView):
      ...
      def post(self, request):   #refresh token sunucuya gönderiliyor
            try:
                  refresh_token = request.data.get('refresh')
                  if not refresh_token:
                        return Response({'error':'Refresh token is required'},status=status.HTTP_400_BAD_REQUEST)
                  token = RefreshToken(refresh_token)
                  token.blacklist()  #bidaa kullanılamaz
                  return Response({'message':'Logout Successful'},status=status.HTTP_205_RESET_CONTENT) #başarılı logout
